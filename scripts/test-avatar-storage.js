...
async function testPublicUrlGeneration() {
  ...
  try {
    ...
    if (uploadError) {
      console.error(
        "❌ Erreur lors du téléchargement pour le test d'URL:",
        uploadError.message
      );
      return false;
    }
    ...
    try {
      ...
      console.warn(
        "⚠️ Impossible de vérifier l'accessibilité de l'URL:",
        fetchError.message
      );
    }
...
async function testFileTypeValidation() {
  ...
  try {
    ...
    if (error) {
      ...
      console.warn(
        '⚠️ La validation des types de fichiers pourrait ne pas être active'
      );
...
async function testFileSizeLimit() {
  ...
  try {
    ...
    if (error) {
      ...
      console.log(
        '   - Erreur attendue pour fichier trop volumineux:',
        error.message
      );
    } else {
      console.warn(
        '⚠️ La limite de taille de fichier pourrait ne pas être active'
      );
      // Nettoyer si le fichier a été téléchargé
      await supabase.storage.from('avatars').remove([filePath]);
    }
...
async function runTests() {
  console.log(
    "🚀 Démarrage des tests de configuration du stockage d'avatars\n"
  );
