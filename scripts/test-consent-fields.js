...
async function testConsentFields() {
  ...
  try {
    ...
    const { data: columns, error: columnsError } = await supabase
      .rpc('get_table_columns', { table_name: 'nutritionists' });

    if (columnsError) {
      // Fallback: essayer une requête simple pour voir si les colonnes existent
      const { data: testData, error: testError } = await supabase
        .from('nutritionists')
        .select('terms_accepted, terms_accepted_at, privacy_policy_accepted, privacy_policy_accepted_at, marketing_consent, marketing_consent_at')
        .limit(1);
      ...
      const consentFields = ['terms_accepted', 'terms_accepted_at', 'privacy_policy_accepted', 'privacy_policy_accepted_at', 'marketing_consent', 'marketing_consent_at'];
      const existingFields = columns.filter(col => consentFields.includes(col.column_name));
      
      console.log(`   Champs de consentement trouvés: ${existingFields.length}/6`);
    ...
    const { data: indexes, error: indexError } = await supabase
      .rpc('get_table_indexes', { table_name: 'nutritionists' });

    if (!indexError && indexes) {
      const consentIndexes = indexes.filter(idx => 
        idx.indexname.includes('terms_accepted') || 
        idx.indexname.includes('privacy_accepted') || 
        idx.indexname.includes('marketing_consent')
      );
      
      console.log(`   Index de consentement trouvés: ${consentIndexes.length}/3`);
      consentIndexes.forEach(idx => {
        console.log(`   ✅ ${idx.indexname}`);
      });
    } else {
      console.log('   ⚠️  Impossible de vérifier les index (normal si RPC n\'existe pas)');
    }
