...
async function createTables() {
  ...
  if (eventsError && eventsError.code === 'PGRST116') {
    ...
    console.log(
      "⚠️  Table onboarding_events n'existe pas. Création nécessaire via l'interface Supabase."
    );
    console.log(
      "📋 Veuillez exécuter le script SQL suivant dans l'interface Supabase :"
    );
    console.log('');
    ...
    console.log('');
    console.log(
      '⚠️  Après avoir créé les tables, relancez ce script pour insérer les données de test.'
    );
    return;
  } else if (eventsError) {
    console.error(
      '❌ Erreur lors de la vérification de la table onboarding_events:',
      eventsError
    );
    return;
  }
