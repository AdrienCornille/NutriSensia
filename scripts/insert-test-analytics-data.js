...
async function insertTestData() {
  try {
    ...
    if (eventsError) {
      ...
      console.log('📋 Utilisez le fichier: scripts/create-analytics-tables-simple.sql');
      return;
    }
  ...
  for (let day = 6; day >= 0; day--) {
    ...
    date.setHours(0, 0, 0, 0); // Commencer au début de la journée
      ...
      for (let user = 0; user < usersPerDay; user++) {
        ...
        // Créer une session
        const sessionStartTime = new Date(date.getTime() + Math.random() * 24 * 60 * 60 * 1000);
        const session = {
        ...
        if (session.status === 'completed') {
          session.completed_at = new Date(new Date(session.started_at).getTime() + Math.random() * 30 * 60 * 1000).toISOString();
          session.completion_percentage = 100;
          session.total_time_spent = Math.floor(Math.random() * 20 + 5) * 60 * 1000; // 5-25 minutes
        } else {
          session.abandoned_at = new Date(new Date(session.started_at).getTime() + Math.random() * 15 * 60 * 1000).toISOString();
