...
async function testDatabaseSchema() {
  ...
  try {
    ...
    if (viewError) {
      console.error(
        '❌ Vue ab_test_conversions_by_variant:',
        viewError.message
      );
      return false;
    }
    ...
    console.log(
      '✅ Tous les schémas de base de données sont correctement déployés'
    );
    return true;
  } catch (error) {
...
async function testEventTracking() {
  ...
  try {
    ...
    await supabase.from('ab_test_events').delete().eq('user_id', testUser.id);
...
async function testAnalyticsAPI() {
  ...
  try {
    ...
    const summaryResponse = await fetch(
      `${BASE_URL}/api/ab-test/analytics?action=summary`,
      {
        headers: {
          Authorization: 'Bearer test-token', // À adapter selon votre auth
        },
    ...
    const resultsResponse = await fetch(
      `${BASE_URL}/api/ab-test/analytics?action=results&flagKey=nutritionist-onboarding-variant`,
      {
        headers: {
    ...
    const metricsResponse = await fetch(
      `${BASE_URL}/api/ab-test/analytics?action=metrics&flagKey=nutritionist-onboarding-variant`,
      {
        headers: {
          Authorization: 'Bearer test-token',
        },
...
async function testFlagAttribution() {
  ...
  try {
    ...
    for (const user of testUsers) {
      ...
      console.log(
        `👤 ${user.role} ${user.id}: ${variant} (hash: ${hashValue})`
      );
    }
...
async function testStatisticalCalculations() {
  ...
  try {
    ...
    testData.forEach(data => {
      ...
      console.log(
        `   ${data.variant}: ${data.conversions}/${data.users} (${rate}%)`
      );
    });
    ...
    try {
      ...
      if (!error && significance && significance.length > 0) {
        ...
        console.log(
          `   Significatif: ${result.is_significant ? 'Oui' : 'Non'}`
        );
        console.log(
          `   Confiance: ${(result.confidence_level * 100).toFixed(1)}%`
        );
      } else {
      ...
      console.log(
        '⚠️  Fonction calculate_statistical_significance non trouvée'
      );
    }
...
async function testPerformance() {
  ...
  try {
    ...
    console.log(
      `📊 Performance: ${((batchSize / insertTime) * 1000).toFixed(0)} événements/seconde`
    );
...
async function runAllTests() {
  ...
  console.log(
    `📈 Taux de réussite: ${((passed / results.length) * 100).toFixed(1)}%`
  );
