...
async function testConsentRealtime() {
  ...
  try {
    ...
    if (!nutritionists || nutritionists.length === 0) {
      ...
      console.log(
        "🔧 Complétez l'onboarding sur http://localhost:3000/onboarding/nutritionist pour créer des données de test."
      );
      return;
    }
    ...
    nutritionists.forEach((nutritionist, index) => {
      console.log(
        `${index + 1}. ${nutritionist.first_name} ${nutritionist.last_name} (ID: ${nutritionist.id.substring(0, 8)}...)`
      );
      console.log(
        `   📋 Conditions d'utilisation: ${nutritionist.terms_accepted ? '✅ Acceptées' : '❌ Non acceptées'}`
      );
      if (nutritionist.terms_accepted_at) {
        console.log(
          `      Acceptées le: ${new Date(nutritionist.terms_accepted_at).toLocaleString('fr-FR')}`
        );
      }

      console.log(
        `   🔒 Politique de confidentialité: ${nutritionist.privacy_policy_accepted ? '✅ Acceptée' : '❌ Non acceptée'}`
      );
      if (nutritionist.privacy_policy_accepted_at) {
        console.log(
          `      Acceptée le: ${new Date(nutritionist.privacy_policy_accepted_at).toLocaleString('fr-FR')}`
        );
      }

      console.log(
        `   📧 Consentement marketing: ${nutritionist.marketing_consent ? '✅ Accepté' : '❌ Refusé'}`
      );
      if (nutritionist.marketing_consent_at) {
        console.log(
          `      Décidé le: ${new Date(nutritionist.marketing_consent_at).toLocaleString('fr-FR')}`
        );
      }

      console.log(
        `   🕒 Dernière mise à jour: ${new Date(nutritionist.updated_at).toLocaleString('fr-FR')}`
      );
      console.log('');
    });
    ...
    console.log('');
    console.log(
      '💡 Astuce: Ouvrez les outils de développement (F12) pour voir les logs de sauvegarde'
    );
  } catch (error) {
