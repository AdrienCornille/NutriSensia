...
async function setupAvatarStorage() {
  try {
    ...
    const { error: selectPolicyError } = await supabase.rpc('create_avatar_select_policy');
    if (selectPolicyError && !selectPolicyError.message.includes('already exists')) {
      console.warn('⚠️  Erreur lors de la création de la politique SELECT:', selectPolicyError.message);
    } else {
      console.log('✅ Politique SELECT configurée');
    }

    // Politique pour permettre l'upload aux utilisateurs authentifiés
    const { error: insertPolicyError } = await supabase.rpc('create_avatar_insert_policy');
    if (insertPolicyError && !insertPolicyError.message.includes('already exists')) {
      console.warn('⚠️  Erreur lors de la création de la politique INSERT:', insertPolicyError.message);
    } else {
      console.log('✅ Politique INSERT configurée');
    }

    // Politique pour permettre la mise à jour aux propriétaires
    const { error: updatePolicyError } = await supabase.rpc('create_avatar_update_policy');
    if (updatePolicyError && !updatePolicyError.message.includes('already exists')) {
      console.warn('⚠️  Erreur lors de la création de la politique UPDATE:', updatePolicyError.message);
    } else {
      console.log('✅ Politique UPDATE configurée');
    }

    // Politique pour permettre la suppression aux propriétaires
    const { error: deletePolicyError } = await supabase.rpc('create_avatar_delete_policy');
    if (deletePolicyError && !deletePolicyError.message.includes('already exists')) {
      console.warn('⚠️  Erreur lors de la création de la politique DELETE:', deletePolicyError.message);
    } else {
