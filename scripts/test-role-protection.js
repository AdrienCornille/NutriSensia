...
async function testRoleProtection() {
  ...
  for (const { role, shouldAccess, description } of testRoles) {
    try {
      ...
      console.log(
        `   📊 API Auth: ${authResponse.status} - Rôle: ${userData.role}, isAdmin: ${userData.isAdmin}`
      );
      ...
      console.log(
        `   ${status} Résultat: ${accessText} ${hasAccess === shouldAccess ? '(attendu)' : '(INATTENDU!)'}`
      );

      if (hasAccess !== shouldAccess) {
        console.log(
          `   ⚠️  PROBLÈME: ${description} ${hasAccess ? 'a accès' : "n'a pas accès"} mais devrait ${shouldAccess ? 'avoir' : 'ne pas avoir'} accès`
        );
      }
    } catch (error) {
      console.error(
        `   ❌ Erreur lors du test du rôle ${role}:`,
        error.message
      );
    }
  }
  ...
  console.log(
    '- Seuls les administrateurs devraient avoir accès aux pages A/B Testing'
  );
}
