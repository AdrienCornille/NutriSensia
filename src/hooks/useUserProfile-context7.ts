      ...
      if (profileData.role === 'nutritionist') {
        try {
          ...
          if (!nutritionistError) {
            ...
            console.log(
              '✅ [Context7] Données nutritionniste chargées:',
              nutritionistData
            );
          } else {
            console.warn(
              '⚠️ [Context7] Erreur nutritionniste:',
              nutritionistError.message
            );
          }
        } catch (error) {
    ...
    async updates => {
      ...
      try {
        ...
        if (Object.keys(roleSpecificUpdates).length > 0) {
          ...
          console.log(
            `🔄 [Context7] Mise à jour ${tableName} avec configuration optimisée`
          );
          console.log(
            '📊 [Context7] Données à sauvegarder:',
            roleSpecificUpdates
          );
          ...
          console.log(
            `📊 [Context7] Lignes affectées: ${updateResult?.length || 0}`
          );
          ...
          if (updateResult && updateResult.length > 0) {
            ...
            console.warn(
              '⚠️ [Context7] Aucune donnée retournée - Vérification nécessaire'
            );
          }
        }
