...
export const useOnboardingProgress = ({
  ...
  const hookInstanceId = React.useRef(Math.random().toString(36).substr(2, 9)).current;
  console.log(`🔧 Hook Instance ${hookInstanceId} - Création pour userId: ${userId}`);
   ...
   */
  const saveToLocalStorage = useCallback((progressData: OnboardingProgress) => {
   ...
   */
  const loadFromDatabase = useCallback(async (): Promise<OnboardingProgress | null> => {
    // Désactivé - pas de chargement depuis la base pour la progression
    console.log('📥 Chargement progression désactivé (localStorage uniquement)');
    return null;
  }, [userId, role]);
   ...
   */
  const saveToDatabase = useCallback(async (progressData: OnboardingProgress) => {
    // Désactivé - pas de sauvegarde en base pour la progression
    console.log('💾 Sauvegarde progression désactivée (localStorage uniquement)');
    return;
  }, []);
   ...
   */
  const calculateCompletionPercentage = useCallback((steps: Record<string, OnboardingStepInfo>): number => {
  ...
  const trackEvent = useCallback(async (eventType: string, data: any) => {
    ...
    try {
      ...
      console.log('Analytics Event:', { eventType, data, timestamp: new Date().toISOString() });
  ...
  const initProgressManually = useCallback(async () => {
    console.log('🔄 Initialisation manuelle de la progression - userId:', userId, 'role:', role);
    ...
    try {
      ...
      if (persistToLocalStorage) {
        try {
          const stored = localStorage.getItem(`onboarding_progress_${userId}_${role}`);
          if (stored) {
            const parsed = JSON.parse(stored);
            if (parsed.userId === userId && parsed.userRole === role) {
      ...
      console.log('🔄 Progression initialisée:', loadedProgress.completionPercentage + '%');
  ...
  useEffect(() => {
    if (userId && !progress && !isInitialized) {
      console.log(`🚀 Hook Instance ${hookInstanceId} - Auto-initialisation de la progression`);
  ...
  }, [progress, autoSave, saveInterval, lastSaveTime, saveToDatabase, saveToLocalStorage]);
  ...
  }, [progress, calculateCompletionPercentage, saveToDatabase, saveToLocalStorage, trackEvent, userId, autoSave]);
  ...
  const completeStep = useCallback(async (step: OnboardingStep, data: Partial<OnboardingData>) => {
    ...
    console.log('📈 completeStep - Mise à jour progression:', {
      ...
      completedSteps: Object.values(updatedProgress.steps).filter(s => s.status === 'completed').length
    });
    ...
    try {
      ...
      if (updatedProgress.isCompleted) {
        await trackEvent('ONBOARDING_COMPLETED', {
          userId,
          totalDuration: Date.now() - new Date(updatedProgress.startedAt).getTime(),
    ...
    // Forcer une nouvelle référence pour déclencher le re-render React
    console.log(`🔄 Hook Instance ${hookInstanceId} - Mise à jour de l'état React avec: ${updatedProgress.completionPercentage}%`);
    setProgress(updatedProgress);
  }, [progress, calculateCompletionPercentage, saveToDatabase, saveToLocalStorage, trackEvent, userId]);

  /**
   * Passer une étape
   */
  const skipStep = useCallback(async (step: OnboardingStep, reason?: string) => {
    if (!progress) return;
  ...
  }, [initializeProgress, saveToDatabase, saveToLocalStorage, persistToLocalStorage, getStorageKey]);
