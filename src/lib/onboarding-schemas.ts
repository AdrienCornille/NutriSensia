...
export const healthProfileSchema = patientDemographicsSchema.merge(
  physicalMeasurementsSchema
);
export const dietaryInfoSchema = dietaryRestrictionsSchema.merge(
...
export const appTourSchema = z.object({
  ...
  communicationPreferences: z.object({
    ...
    reminderFrequency: z
      .enum(['none', 'daily', 'weekly', 'bi-weekly'])
      .default('weekly'),
  }),
  privacySettings: z.object({
    ...
    profileVisibility: z
      .enum(['private', 'nutritionist-only', 'community'])
      .default('nutritionist-only'),
  }),
});
...
export const getStepSchema = (role: UserRole, step: string) => {
  switch (role) {
    case 'nutritionist':
      return nutritionistStepSchemas[
        step as keyof typeof nutritionistStepSchemas
      ];
    case 'patient':
 ...
 */
export type NutritionistOnboardingData = z.infer<
  typeof nutritionistOnboardingSchema
>;
