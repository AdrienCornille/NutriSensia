...
export class MFACleanupUtil {
  ...
  static async cleanupUnverifiedFactors(
    ...
    try {
      ...
      if (listError) {
        result.errors.push(`Erreur lors de la liste des facteurs: ${listError.message}`);
        return result;
      }
      ...
      if (forceCleanVerified) {
        ...
        console.warn('⚠️ MODE FORCE ACTIVÉ : Nettoyage de TOUS les facteurs MFA');
        factorsToClean = [
        ...
        factorsToClean = allFactors.filter(f => {
          ...
          if (createdAt > fifteenMinutesAgo) {
            console.log(`🛡️ Facteur récent préservé: ${f.id} (créé il y a ${Math.round((Date.now() - createdAt.getTime()) / 1000 / 60)} minutes)`);
            return false;
          }
          
          // Préserver les facteurs avec un nom (probablement en cours d'utilisation)
          if (f.friendly_name && f.friendly_name.trim() !== '') {
            console.log(`🛡️ Facteur avec nom préservé: ${f.id} (${f.friendly_name})`);
            return false;
          }
      ...
      console.log(`📋 ${unverifiedFactors.length} facteurs non vérifiés trouvés`);
      ...
      for (const factor of unverifiedFactors) {
        try {
          console.log(`🗑️ Suppression du facteur: ${factor.id} (${factor.factor_type})`);
          ...
          if (unenrollError) {
            result.errors.push(`Erreur suppression ${factor.id}: ${unenrollError.message}`);
      ...
      console.log(`🎉 Nettoyage terminé: ${result.cleaned} facteurs supprimés, ${result.errors.length} erreurs`);
  ...
  static async getFactorsCount(): Promise<{
    ...
    try {
      ...
      const unverified = allFactors.filter(f => f.status === 'unverified').length;
