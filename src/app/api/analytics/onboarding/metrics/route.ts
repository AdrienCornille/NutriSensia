...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log(`🔧 Paramètres: timeframe=${timeframe}, role=${role}, type=${type}`);
    ...
    switch (type) {
      case 'overview':
        ...
        if (overviewError) {
          console.error('Erreur lors de la récupération des données overview:', overviewError);
          ...
          const completedUsers = overviewData.filter(s => s.status === 'completed').length;
          const abandonedUsers = overviewData.filter(s => s.status === 'abandoned').length;
          const completionRate = totalUsers > 0 ? (completedUsers / totalUsers) * 100 : 0;
          const abandonmentRate = totalUsers > 0 ? (abandonedUsers / totalUsers) * 100 : 0;
          const averageTimeToComplete = completedUsers > 0 
            ? overviewData
                .filter(s => s.status === 'completed' && s.total_time_spent)
                .reduce((sum, s) => sum + (s.total_time_spent || 0), 0) / completedUsers
            : 0;
          const currentActiveUsers = overviewData.filter(s => s.status === 'active').length;
      ...
      case 'funnel':
        ...
        const { data: funnelData, error: funnelError } = await supabase
          ...
          .in('event_type', ['onboarding_started', 'step_started', 'step_completed', 'onboarding_completed', 'step_error', 'onboarding_abandoned']);

        if (funnelError) {
          console.error('Erreur lors de la récupération des données funnel:', funnelError);
          // Fallback vers des données de test
          data = [
            { step: 'Bienvenue', step_number: 1, sessions_entered: 100, sessions_completed: 95, completion_rate: 95.0, drop_off_rate: 5.0, average_time_spent: 30000 },
            { step: 'Profil', step_number: 2, sessions_entered: 95, sessions_completed: 85, completion_rate: 89.47, drop_off_rate: 10.53, average_time_spent: 120000 },
            { step: 'Spécialisations', step_number: 3, sessions_entered: 85, sessions_completed: 75, completion_rate: 88.24, drop_off_rate: 11.76, average_time_spent: 90000 },
            { step: 'Tarifs', step_number: 4, sessions_entered: 75, sessions_completed: 65, completion_rate: 86.67, drop_off_rate: 13.33, average_time_spent: 60000 },
            { step: 'Finalisation', step_number: 5, sessions_entered: 65, sessions_completed: 60, completion_rate: 92.31, drop_off_rate: 7.69, average_time_spent: 45000 }
          ];
        } else {
          ...
          for (let i = 0; i < stepNumbers.length; i++) {
            ...
            const stepEvents = funnelData.filter(e => e.step_number === stepNumber);
            ...
            if (i === 0) {
              ...
              sessionsEntered = funnelData.filter(e => 
                e.step_number === stepNumber && e.event_type === 'onboarding_started'
              ).length;
            } else {
            ...
            const stepTimeEvents = funnelData.filter(e => 
              e.step_number === stepNumber && e.time_spent
            );
            const averageTimeSpent = stepTimeEvents.length > 0 
              ? stepTimeEvents.reduce((sum, e) => sum + (e.time_spent || 0), 0) / stepTimeEvents.length
              : 0;
            
            // Calculer les taux
            const completionRate = sessionsEntered > 0 ? (sessionsCompleted / sessionsEntered) * 100 : 0;
            const dropOffRate = sessionsEntered > 0 ? ((sessionsEntered - sessionsCompleted) / sessionsEntered) * 100 : 0;
      ...
      case 'errors':
        ...
        if (errorsError) {
          console.error('Erreur lors de la récupération des données d\'erreurs:', errorsError);
          // Fallback vers des données de test
          data = [
            { step: 'Profil', stepNumber: 2, errorType: 'validation', count: 12 },
            { step: 'Spécialisations', stepNumber: 3, errorType: 'network', count: 8 },
            { step: 'Tarifs', stepNumber: 4, errorType: 'validation', count: 5 },
            { step: 'Finalisation', stepNumber: 5, errorType: 'server', count: 3 }
          ];
        } else {
          // Grouper les erreurs par étape et type
          const errorGroups = errorsData.reduce((acc, event) => {
      ...
      case 'help':
        ...
        if (helpError) {
          console.error('Erreur lors de la récupération des données d\'aide:', helpError);
          // Fallback vers des données de test
          data = [
            { step: 'Spécialisations', stepNumber: 3, helpType: 'tooltip', count: 15 },
          ...
          const helpGroups = helpData.reduce((acc, event) => {
      ...
      case 'dashboard':
        ...
        const [overviewResult, funnelResult, errorsResult, helpResult] = await Promise.all([
          overviewQuery,
          funnelQuery,
          errorsQuery,
          helpQuery
        ...
        if (overviewResult.error) {
          console.error('Erreur lors de la récupération des données overview:', overviewResult.error);
          overview = {
            totalUsers: 42,
            completedUsers: 28,
            abandonedUsers: 8,
        ...
        if (funnelResult.error) {
          console.error('Erreur lors de la récupération des données funnel:', funnelResult.error);
          funnel = [
            { step: 'Bienvenue', step_number: 1, sessions_entered: 100, sessions_completed: 95, completion_rate: 95.0, drop_off_rate: 5.0, average_time_spent: 30000 },
            { step: 'Profil', step_number: 2, sessions_entered: 95, sessions_completed: 85, completion_rate: 89.47, drop_off_rate: 10.53, average_time_spent: 120000 },
            { step: 'Spécialisations', step_number: 3, sessions_entered: 85, sessions_completed: 75, completion_rate: 88.24, drop_off_rate: 11.76, average_time_spent: 90000 },
        ...
        if (errorsResult.error) {
          console.error('Erreur lors de la récupération des données d\'erreurs:', errorsResult.error);
          errors = [
            { step: 'Profil', stepNumber: 2, errorType: 'validation', count: 12 },
            { step: 'Spécialisations', stepNumber: 3, errorType: 'network', count: 8 },
            { step: 'Tarifs', stepNumber: 4, errorType: 'validation', count: 5 },
        ...
        let helpRequests;
        if (helpResult.error) {
          console.error('Erreur lors de la récupération des données d\'aide:', helpResult.error);
          helpRequests = [
            { step: 'Spécialisations', stepNumber: 3, helpType: 'tooltip', count: 15 },
            { step: 'Tarifs', stepNumber: 4, helpType: 'faq', count: 10 },
            { step: 'Profil', stepNumber: 2, helpType: 'video', count: 8 },
        ...
        try {
          ...
          if (trendsResult.error) {
            console.error('Erreur lors de la récupération des tendances:', trendsResult.error);
            ...
            // Grouper par jour
            const dailyData = trendsData.reduce((acc, event) => {
            ...
            trends = Object.values(dailyData)
              .map(day => ({
                ...day,
                completionRate: day.usersStarted > 0 ? Math.round((day.usersCompleted / day.usersStarted) * 100) : 0
              }))
    ...
    console.error('❌ Erreur dans GET /api/analytics/onboarding/metrics:', error);
    return NextResponse.json(
