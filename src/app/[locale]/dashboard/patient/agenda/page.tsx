...
export default function AgendaPage() {
  ...
  // Map UI consultation type names to database IDs
  const consultationTypeIdMap = useMemo(() => {
    if (!consultationTypes) return {};
    return consultationTypes.reduce((acc, type) => {
      // Map DB codes to UI names
      // DB: 'follow_up' -> UI: 'suivi'
      // DB: 'in_depth' -> UI: 'approfondi'
      // DB: 'emergency' -> UI: 'urgence'
      const uiNameMap: Record<string, string> = {
        follow_up: 'suivi',
        in_depth: 'approfondi',
        emergency: 'urgence',
        initial: 'initial', // Pour référence
      };
      const uiName = uiNameMap[type.code];
      if (uiName) {
        acc[uiName] = type.id;
      }
      return acc;
  ...
  const handleConfirmCancellation = useCallback(async (appointment: Appointment) => {
    try {
  ...
  const handleBookingSubmit = useCallback(async (formData: BookingFormData) => {
    ...
    if (!formData.selectedDate || !formData.selectedTime || !formData.mode || !formData.consultationType) {
      dispatch({
    ...
    try {
      if (isEditMode && state.editingAppointment) {
        ...
        const updateData: { scheduled_at?: string; mode?: 'visio' | 'cabinet' | 'phone'; patient_notes?: string } = {
        ...
        // Get consultation type ID from the fetched types
        const consultationTypeId = consultationTypeIdMap[formData.consultationType];

        if (!consultationTypeId) {
          throw new Error(`Type de consultation "${formData.consultationType}" non trouvé. Veuillez réessayer.`);
        }
        ...
        const newAppointment = await createAppointmentMutation.mutateAsync(createData);

        // Transform and add to local state
        const transformedAppointment = transformAppointmentToUI(newAppointment);
        dispatch({ type: 'ADD_APPOINTMENT', appointment: transformedAppointment });
        dispatch({ type: 'CLOSE_BOOKING_MODAL' });
  ...
  }, [state.editingAppointment, createAppointmentMutation, updateAppointmentMutation, consultationTypeIdMap]);
      ...
      return (
        <div className="bg-red-50 border border-red-200 rounded-xl p-6 text-center">
          ...
          <p className="text-red-500 text-sm mt-2">
            {upcomingError?.message || pastError?.message}
          </p>
          <button
