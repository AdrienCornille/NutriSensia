...
export default function NutritionistOnboardingPage() {
  ...
  const [initialData, setInitialData] = useState<Partial<NutritionistOnboardingData>>({});
    ...
    const checkUserAccess = async () => {
      ...
      try {
        ...
        const initialData: Partial<NutritionistOnboardingData> = {
          ...
          consultationTypes: nutritionistInfo.consultation_types || ['initial', 'suivi', 'express'],
          ...
          privacyPolicyAccepted: nutritionistInfo.privacy_policy_accepted || false,
   ...
   */
  const handleOnboardingComplete = useCallback(async (data: NutritionistOnboardingData) => {
    try {
      ...
      showSuccess('Onboarding terminé !', 'Votre profil professionnel a été créé avec succès. Un administrateur va vérifier vos informations. Vous pouvez continuer à modifier votre profil si nécessaire.');
      
    } catch (error) {
      showError('Erreur de sauvegarde', 'Une erreur est survenue lors de la finalisation. Veuillez réessayer.');
      throw error;
    }
   ...
   */
  const handleProgressSave = useCallback(async (data: Partial<NutritionistOnboardingData>) => {
    try {
      if (!user) {
        return;
      }
      ...
      if (data.avatar_url !== undefined && hasFieldChanged('avatar_url', data.avatar_url)) {
      ...
      if (data.ascaNumber && data.ascaNumber.trim() !== '') {
        ...
        try {
          ...
          if (ascaResults && ascaResults.length > 0) {
            showError('Numéro ASCA déjà utilisé', 'Ce numéro ASCA est déjà utilisé par un autre nutritionniste. Veuillez vérifier votre numéro.');
      ...
      if (data.rmeNumber && data.rmeNumber.trim() !== '') {
        ...
        try {
          ...
          if (rmeResults && rmeResults.length > 0) {
            showError('Numéro RME déjà utilisé', 'Ce numéro RME est déjà utilisé par un autre nutritionniste. Veuillez vérifier votre numéro.');
      ...
      if (data.eanCode && data.eanCode.trim() !== '') {
        ...
        try {
          ...
          if (eanResults && eanResults.length > 0) {
            showError('Code EAN déjà utilisé', 'Ce code EAN est déjà utilisé par un autre nutritionniste. Veuillez vérifier votre code.');
      ...
      if (data.specializations && hasFieldChanged('specializations', data.specializations)) {
      ...
      if (data.yearsOfExperience !== undefined && hasFieldChanged('years_of_experience', data.yearsOfExperience)) {
        nutritionistUpdate.years_of_experience = data.yearsOfExperience;
        hasAnyData = true;
      }
      if (data.certifications && hasFieldChanged('certifications', data.certifications)) {
        nutritionistUpdate.certifications = data.certifications;
        hasAnyData = true;
      }
      if (data.continuingEducation !== undefined && hasFieldChanged('continuing_education', data.continuingEducation)) {
        nutritionistUpdate.continuing_education = data.continuingEducation;
        hasAnyData = true;
      }
      if (data.consultationRates && hasFieldChanged('consultation_rates', data.consultationRates)) {
        nutritionistUpdate.consultation_rates = data.consultationRates;
        hasAnyData = true;
      }
      if (data.consultationTypes && hasFieldChanged('consultation_types', data.consultationTypes)) {
        nutritionistUpdate.consultation_types = data.consultationTypes;
        hasAnyData = true;
      }
      if (data.practiceAddress && hasFieldChanged('practice_address', data.practiceAddress)) {
        nutritionistUpdate.practice_address = data.practiceAddress;
        hasAnyData = true;
      }
      if (data.maxPatients !== undefined && hasFieldChanged('max_patients', data.maxPatients)) {
      ...
      if (data.privacyPolicyAccepted !== undefined) {
        nutritionistUpdate.privacy_policy_accepted = data.privacyPolicyAccepted;
        if (data.privacyPolicyAccepted) {
          nutritionistUpdate.privacy_policy_accepted_at = new Date().toISOString();
        }
        hasAnyData = true;
        console.log('💾 Sauvegarde privacyPolicyAccepted:', data.privacyPolicyAccepted);
      }
      ...
      if (hasAnyData) {
        ...
        if (!nutritionistUpdate.is_active) nutritionistUpdate.is_active = true;
        if (!nutritionistUpdate.profile_public) nutritionistUpdate.profile_public = false;
        if (!nutritionistUpdate.allow_contact) nutritionistUpdate.allow_contact = true;
        ...
        const { data: existingNutritionist, error: checkError } = await supabase
          .from('nutritionists')
          .select('id')
          .eq('id', user.id)
          .single();
        ...
        if (checkError && checkError.code === 'PGRST116') {
          ...
          console.log('🆕 Création nouveau nutritionniste avec données:', nutritionistUpdate);
          ...
          console.log('🔄 Mise à jour nutritionniste avec données:', nutritionistUpdate);
        ...
        if (nutritionistError) {
          ...
          if (nutritionistError.code === '23505') {
            ...
            if (nutritionistError.message.includes('rme_number')) {
              ...
              showError('Données déjà utilisées', 'Certaines de vos données professionnelles sont déjà utilisées par un autre nutritionniste.');
            }
            
            // IMPORTANT: Lancer une exception pour empêcher la progression
            throw new Error(`Contrainte d'unicité violée: ${nutritionistError.message}`);
          } else {
            // Pour les autres erreurs, lancer une exception générique
            throw new Error(`Erreur de sauvegarde: ${nutritionistError.message}`);
          }
        }
      }
