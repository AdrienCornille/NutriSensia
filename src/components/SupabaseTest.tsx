...
export function SupabaseTest() {
  ...
  const testAuth = async () => {
    ...
    try {
      ...
      const { data, error } = await signUp(testEmail, 'testpassword123', { name: 'Test User' })
      
      if (error) {
        setTestResult(`❌ Erreur d'authentification: ${error instanceof Error ? error.message : 'Erreur inconnue'}`)
      } else {
      ...
      <div className="space-y-4">
        <div>
          <p className="text-sm text-neutral mb-2">
            Statut de l'authentification: {isAuthenticated ? '✅ Connecté' : '❌ Non connecté'}
          </p>
          {user && (
