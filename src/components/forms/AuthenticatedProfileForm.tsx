...
export function AuthenticatedProfileForm({ 
  ...
  const { watch, formState: { isDirty, isSubmitting, errors } } = methods;
    ...
    return (
      <div className="max-w-4xl mx-auto p-6">
        <Alert variant="error" title="Rôle utilisateur invalide">
          <p>Votre compte n'a pas de rôle valide. Veuillez contacter l'administrateur.</p>
        </Alert>
      </div>
    );
  }
    ...
    <AuthGuard>
      <div className="max-w-4xl mx-auto p-6">
        ...
        >
          ...
          {saveError && (
            <Alert variant="error" title="Erreur de sauvegarde" className="mb-6">
              <p>{saveError}</p>
            </Alert>
          )}
