...
export const ProfileOnboardingGuide = ({
  ...
  const generateSteps = (): OnboardingStep[] => {
    ...
    const basicCompleted = basicFields.every(
      field =>
        completion.missingFields.critical
          .concat(
            completion.missingFields.important,
            completion.missingFields.optional
          )
    ...
    if (role === 'nutritionist') {
      ...
      const profCompleted = profFields.some(
        field =>
          completion.missingFields.critical
            .concat(completion.missingFields.important)
            .find(f => f.name === field) === undefined
      );
      ...
      const contactCompleted = contactFields.every(
        field =>
          completion.missingFields.critical
            .concat(completion.missingFields.important)
            .find(f => f.name === field) === undefined
      ...
      const medicalCompleted = medicalFields.every(
        field =>
          completion.missingFields.critical.find(f => f.name === field) ===
          undefined
      );
      ...
      const goalsCompleted = goalsFields.every(
        field =>
          completion.missingFields.important.find(f => f.name === field) ===
          undefined
      );
