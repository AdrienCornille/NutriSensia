...
const getProfileSummary = (data: Partial<PatientOnboardingData>) => {
  ...
  if (data.primaryGoals?.length) {
    items.push(`${data.primaryGoals.length} objectif${data.primaryGoals.length > 1 ? 's' : ''} principal${data.primaryGoals.length > 1 ? 'aux' : ''}`);
  }
  
  if (data.dietaryRestrictions?.length) {
    items.push(`${data.dietaryRestrictions.length} restriction${data.dietaryRestrictions.length > 1 ? 's' : ''} alimentaire${data.dietaryRestrictions.length > 1 ? 's' : ''}`);
  }
  
  if (data.allergies?.length) {
    items.push(`${data.allergies.length} allergie${data.allergies.length > 1 ? 's' : ''}`);
  }
  ...
  if (data.activityLevel) {
    ...
    items.push(`profil ${levelLabels[data.activityLevel as keyof typeof levelLabels]}`);
  }
  ...
  return (
    <div className="space-y-8">
      ...
      >
        ...
        >
          Voici un récapitulatif de toutes les informations que vous avez saisies 
          lors de votre onboarding. Vous êtes prêt à commencer votre parcours !
        </motion.p>
      </motion.div>
      ...
      >
        <WizardTip type="success" title="Vous êtes sur la bonne voie !">
          <div className="space-y-2 text-sm">
            <p>
              <strong>Bravo</strong> pour avoir pris cette décision importante pour votre santé. 
              Notre équipe de nutritionnistes qualifiés est maintenant prête à vous accompagner.
            </p>
