...
import { Smartphone, Bell, Mail, MessageSquare, Settings, Play } from 'lucide-react';
...
export const AppTourStep: React.FC<AppTourStepProps> = ({
  ...
  const { register, handleSubmit, formState: { errors, isValid }, watch } = useForm({
    ...
    defaultValues: {
      ...
      communicationPreferences: {
        preferredLanguage: data.communicationPreferences?.preferredLanguage || 'fr',
        preferredContactMethod: data.communicationPreferences?.preferredContactMethod || 'app',
        reminderFrequency: data.communicationPreferences?.reminderFrequency || 'weekly',
      },
      privacySettings: {
        shareDataForResearch: data.privacySettings?.shareDataForResearch ?? false,
        allowMarketingEmails: data.privacySettings?.allowMarketingEmails ?? false,
        profileVisibility: data.privacySettings?.profileVisibility || 'nutritionist-only',
      },
    },
    mode: 'onChange',
  });
  ...
  const prevFeature = () => {
    setCurrentFeature((prev) => (prev - 1 + APP_FEATURES.length) % APP_FEATURES.length);
  };
  ...
  return (
    <div className="space-y-6">
      <motion.div className="text-center space-y-2" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <h1 className="text-3xl font-bold text-gray-900">Découvrez NutriSensia</h1>
        <p className="text-gray-600">Explorez les fonctionnalités qui vous accompagneront</p>
      </motion.div>
      ...
      <motion.form onSubmit={handleSubmit(onSubmit)} className="space-y-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
        ...
        <div className="bg-white rounded-lg border p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex space-x-2">
              <Button type="button" variant="secondary" onClick={prevFeature} disabled={currentFeature === 0}>
                ←
              </Button>
              <Button type="button" variant="secondary" onClick={nextFeature} disabled={currentFeature === APP_FEATURES.length - 1}>
                →
              </Button>
            </div>
          </div>
        ...
        <WizardTip type="tip" title="Personnalisez votre expérience">
          <p className="text-sm">
            Vous pourrez modifier tous ces paramètres à tout moment depuis 
