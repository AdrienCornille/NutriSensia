    ...
    <div className='space-y-32dp'>
      ...
      >
        ...
        <p className='text-body-large text-neutral-medium dark:text-neutral-medium max-w-4xl mx-auto'>
          Nous sommes ravis de vous accueillir parmi notre communauté de
          professionnels de la nutrition. NutriSensia va transformer votre
      ...
      >
        {PLATFORM_STATS.map((stat, index) => (
          <div
            key={index}
            className='text-center p-16dp bg-background-primary dark:bg-background-secondary rounded-12dp shadow-card-primary border border-neutral-border dark:border-neutral-border'
          >
            <div className='text-h3 font-bold text-primary dark:text-primary'>
              {stat.value}
            </div>
            <div className='text-body-small text-neutral-medium dark:text-neutral-medium'>
              {stat.label}
            </div>
          </div>
        ))}
      ...
      >
        <p>
          🚀 Prêt à révolutionner votre pratique nutritionnelle ? Commençons
          ensemble ce voyage !
        </p>
      </motion.div>
    </div>
