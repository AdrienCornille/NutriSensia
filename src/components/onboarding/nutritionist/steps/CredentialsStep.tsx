...
export const CredentialsStep: React.FC<CredentialsStepProps> = ({
  ...
  useEffect(() => {
    ...
    watchedFields.ascaNumber,
    watchedFields.rmeNumber,
    watchedFields.eanCode,
  ]);
  ...
  const onSubmit = async (formData: CredentialsFormData) => {
    try {
      ...
      console.log(
        `📤 [CredentialsStep] Données complètes transmises:`,
        updatedData
      );
      ...
      console.error(
        `❌ [CredentialsStep] Erreur lors de la soumission:`,
        error
      );
