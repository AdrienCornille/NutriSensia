...
export const ConsultationRatesStep: React.FC<ConsultationRatesStepProps> = ({
  ...
  const onSubmit = (formData: ConsultationRatesFormData) => {
    ...
    console.log(`🚀 [ConsultationRatesStep] Soumission avec données:`, formData);
    console.log(`📤 [ConsultationRatesStep] Données complètes transmises:`, updatedData);
  ...
  const applySuggestedRates = () => {
    setValue('initial', chfToCentimes(SUGGESTED_RATES.initial.average), { shouldValidate: true });
    setValue('follow_up', chfToCentimes(SUGGESTED_RATES.follow_up.average), { shouldValidate: true });
    setValue('express', chfToCentimes(SUGGESTED_RATES.express.average), { shouldValidate: true });
  };
      ...
      >
        ...
        <div className="grid md:grid-cols-3 gap-4 text-sm">
          {Object.entries(SUGGESTED_RATES).map(([type, rates]) => (
            <div key={type} className="bg-white rounded-lg p-3 border border-blue-100">
              <h4 className="font-medium text-gray-900 mb-1">
                {type === 'initial' ? 'Initiale' : type === 'follow_up' ? 'Suivi' : 'Express'}
              </h4>
              <p className="text-xs text-gray-600 mb-2">{rates.description}</p>
              <div className="space-y-1">
                <p>Moyenne: <strong>CHF {rates.average}</strong></p>
      ...
      >
        ...
        <div>
          ...
          <PriceInput
            ...
            onChange={(centimes) => setValue('initial', centimes, { shouldValidate: true })}
            min={50}
            max={500}
        ...
        <div>
          ...
          <PriceInput
            ...
            onChange={(centimes) => setValue('follow_up', centimes, { shouldValidate: true })}
            min={30}
            max={400}
        ...
        <div>
          ...
          <PriceInput
            ...
            onChange={(centimes) => setValue('express', centimes, { shouldValidate: true })}
            min={20}
            max={300}
        ...
        >
          ...
          <div className="text-2xl font-bold text-green-800 mb-2">
            CHF {calculateMonthlyRevenue().toLocaleString('fr-CH', { minimumFractionDigits: 0 })}
          </div>
      ...
      >
        {Object.entries(SUGGESTED_RATES).map(([type, rates]) => {
          const currentRate = centimesToChf(
            type === 'initial' ? watchedFields.initial :
            type === 'follow_up' ? watchedFields.follow_up :
            watchedFields.express
          );
          const difference = currentRate - rates.average;
          const percentDifference = Math.round((difference / rates.average) * 100);
          
          return (
            <div key={type} className="text-center p-4 bg-white rounded-lg shadow-sm border">
              ...
              <div className={`text-sm flex items-center justify-center ${
                ...
                <TrendingUp className={`h-3 w-3 mr-1 ${difference < 0 ? 'rotate-180' : ''}`} />
                {difference > 0 ? '+' : ''}{percentDifference}% vs moyenne
              </div>
            </div>
          );
        })}
