...
export function WeightSummaryCards({ data }: WeightSummaryCardsProps) {
  const progress = calculateWeightProgress(data.initial, data.current, data.goal);
  ...
  const isPositiveChange = (data.change < 0 && isGoalLoss) || (data.change > 0 && !isGoalLoss);
  const isPositiveTrend = (data.weeklyChange < 0 && isGoalLoss) || (data.weeklyChange > 0 && !isGoalLoss);
    ...
    <div className="grid grid-cols-4 gap-4">
      ...
      <div className="bg-white rounded-xl p-4 border border-gray-200">
        ...
        <div className="flex items-baseline gap-1 mb-2">
          <span className="text-2xl font-bold text-gray-800">{data.current.toFixed(1)}</span>
      ...
      <div className="bg-white rounded-xl p-4 border border-gray-200">
        ...
        <div className="flex items-baseline gap-1 mb-2">
          <span className="text-2xl font-bold text-[#1B998B]">{data.goal.toFixed(1)}</span>
          <span className="text-sm text-gray-500">kg</span>
        </div>
      ...
      <div className="bg-white rounded-xl p-4 border border-gray-200">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-medium text-gray-600">Tendance hebdo</span>
          <span
        ...
        <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(Math.abs(data.weeklyChange) / 0.5 * 100, 100)}%` }}
          />
        </div>
      </div>
    </div>
