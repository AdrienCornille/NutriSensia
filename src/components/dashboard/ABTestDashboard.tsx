        ...
        {tests.map(test => (
          ...
          >
            ...
            <div className='mt-2 text-xs text-gray-500'>
              {test.variants.length} variantes • Dernière activité:{' '}
              {formatRelativeTime(test.lastActivity)}
            </div>
          </motion.div>
        ))}
      </div>
...
function ActionRecommendation({ action }: { action: string }) {
  ...
  const rec =
    recommendations[action as keyof typeof recommendations] ||
    recommendations.continue;

  return (
    <span
      className={`px-3 py-1 bg-${rec.color}-100 text-${rec.color}-800 text-sm rounded-full font-medium`}
    >
      {rec.text}
    </span>
  );
