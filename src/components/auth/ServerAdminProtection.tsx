...
async function checkServerAdminRole(): Promise<boolean> {
  try {
    ...
    const adminRoles = ['admin', 'super_admin', 'administrator', 'superadmin', 'admin_user', 'system_admin'];
    ...
    console.log(`🔐 [ServerAdminProtection] Vérification du rôle côté serveur: ${normalizedRole}`);
    ...
    console.error('🔐 [ServerAdminProtection] Erreur lors de la vérification du rôle:', error);
    return false;
  }
}
...
export default async function ServerAdminProtection({ 
  ...
  console.log('🔐 [ServerAdminProtection] Vérification des permissions côté serveur');
  
  // Vérifier les permissions administrateur côté serveur
  const isAdmin = await checkServerAdminRole();
  
  if (!isAdmin) {
    console.log('🔐 [ServerAdminProtection] Accès refusé - redirection vers access-denied');
    
    // Rediriger vers la page d'accès refusé
    redirect('/access-denied?reason=admin_required&required_role=admin&current_role=unknown');
  }
  
  console.log('🔐 [ServerAdminProtection] Accès autorisé - affichage du contenu');
