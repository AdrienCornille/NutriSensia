...
import { runMFARedirectionTests, showMFAChanges } from '@/utils/test-mfa-redirection';
...
export const MFADiagnosticAdvanced: React.FC = () => {
  ...
  const forceCleanup = async () => {
    ...
    try {
      ...
      setCleanupResult(`Nettoyage terminé: ${result.cleaned} facteurs supprimés. Erreurs: ${result.errors.length}`);
      
      // Recharger les données
      await loadDiagnosticData();
    } catch (error) {
      setCleanupResult(`Erreur: ${error instanceof Error ? error.message : 'Erreur inconnue'}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const runTests = () => {
    ...
    setCleanupResult('Tests de redirection exécutés - Consultez la console (F12)');
  };
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="bg-gray-50 p-3 rounded text-sm">
            <p><strong>Utilisateur:</strong> {sessionData?.session?.user?.email || 'Non connecté'}</p>
            <p><strong>ID:</strong> {sessionData?.session?.user?.id || 'N/A'}</p>
            <p><strong>Erreur:</strong> {sessionData?.error?.message || 'Aucune'}</p>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="bg-gray-50 p-3 rounded text-sm">
            <p><strong>Niveau actuel:</strong> {aalData?.aal?.current_level || 'N/A'}</p>
            <p><strong>Niveau suivant:</strong> {aalData?.aal?.next_level || 'N/A'}</p>
            <p><strong>Erreur:</strong> {aalData?.error?.message || 'Aucune'}</p>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="bg-gray-50 p-3 rounded text-sm space-y-2">
            <p><strong>Total:</strong> {allFactors.length}</p>
            <p><strong>Vérifiés:</strong> {verifiedFactors.length}</p>
            <p><strong>Non vérifiés:</strong> {unverifiedFactors.length}</p>
            
            {allFactors.length > 0 && (
              <div className="mt-3">
                <p className="font-medium mb-2">Détails des facteurs:</p>
                {allFactors.map((factor, index) => (
                  <div key={factor.id} className="ml-4 mb-2 p-2 bg-white rounded border">
                    <p><strong>#{index + 1}</strong></p>
                    <p><strong>ID:</strong> {factor.id}</p>
                    <p><strong>Type:</strong> {factor.factor_type}</p>
                    <p><strong>Nom:</strong> {factor.friendly_name}</p>
                    ...
                    {factor.created_at && (
                      <p><strong>Créé:</strong> {new Date(factor.created_at).toLocaleString()}</p>
                    )}
                  </div>
                ))}
              </div>
        ...
        <div>
          ...
          <div className="bg-gray-50 p-3 rounded text-sm">
            <p><strong>2FA Activé (DB):</strong> {profileData?.profile?.two_factor_enabled ? 'Oui' : 'Non'}</p>
            <p><strong>Rôle:</strong> {profileData?.profile?.role || 'N/A'}</p>
            <p><strong>Erreur:</strong> {profileData?.error?.message || 'Aucune'}</p>
          </div>
        </div>
          ...
          <div className="bg-blue-50 p-3 rounded text-sm">
            {verifiedFactors.length > 0 && (
              <div className="mb-2 p-2 bg-red-100 border border-red-200 rounded">
                ...
                <p className="text-red-700">
                  {verifiedFactors.length} facteur(s) "vérifiés" trouvé(s) pour un nouveau compte.
                  Cela peut être dû à des données résiduelles ou un bug.
                </p>
              </div>
            )}
            ...
            {unverifiedFactors.length > 0 && (
              <div className="mb-2 p-2 bg-yellow-100 border border-yellow-200 rounded">
                <p className="text-yellow-800">
                  ℹ️ {unverifiedFactors.length} facteur(s) non vérifiés détectés (normal si enrôlement en cours)
                </p>
              </div>
            )}
            ...
            {allFactors.length === 0 && (
              <div className="mb-2 p-2 bg-green-100 border border-green-200 rounded">
                <p className="text-green-800">✅ Aucun facteur MFA détecté (compte propre)</p>
              </div>
            )}
          </div>
        </div>
