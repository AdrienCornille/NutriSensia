...
export function MFAEnrollment({
  ...
  const initializeEnrollment = async () => {
    try {
      ...
      if (!session) {
        throw new Error(
          "Utilisateur non connecté. Veuillez vous connecter d'abord."
        );
      }

      console.log(
        "🔐 Initialisation de l'enrôlement MFA pour:",
        session.user.email
      );
      ...
      console.log(
        '⚠️ Nettoyage préventif désactivé temporairement pour éviter les problèmes de timing'
      );
      ...
      */
      ...
      if (factorsError) {
        ...
        const verifiedPhoneFactors =
          factorsData.phone?.filter(f => f.status === 'verified') || [];
        const hasVerifiedFactors =
          verifiedTotpFactors.length > 0 || verifiedPhoneFactors.length > 0;
        ...
        console.log('🔍 Analyse des facteurs vérifiés:', {
          ...
          phoneFactors: factorsData.phone?.map(f => ({
            id: f.id,
            status: f.status,
            friendly_name: f.friendly_name,
          })),
          allFactorsDetails: allFactors.map(f => ({
            id: f.id,
            status: f.status,
            friendly_name: f.friendly_name,
        ...
        const totpPhoneCount =
          (factorsData.totp?.length || 0) + (factorsData.phone?.length || 0);
        const hasGhostFactors = allFactors.length > totpPhoneCount;

        if (hasGhostFactors) {
          const ghostFactors = allFactors.filter(
            f =>
          ...
          console.warn('👻 Facteurs fantômes détectés:', {
            ...
            ghostFactorsDetails: ghostFactors.map(f => ({
              ...
              age_minutes: Math.round(
                (Date.now() - new Date(f.created_at).getTime()) / 60000
              ),
            })),
          });
          ...
          console.log(
            '🧹 Nettoyage forcé de TOUS les facteurs fantômes (pour éviter "already exists")...'
          );
          // Protéger le facteur actuel s'il existe déjà
          const cleanupResult = await MFACleanupUtil.cleanupUnverifiedFactors(
            true,
            factorId
          );
          console.log('🧹 Nettoyage forcé effectué:', cleanupResult);
          setCleanupResult(
            `Nettoyage des facteurs fantômes: ${cleanupResult.cleaned} facteurs supprimés.`
          );
        ...
        if (hasVerifiedFactors) {
          ...
          if (isRecentAccount && twoFactorDisabledInDB) {
            console.warn(
              '⚠️ Compte récent avec facteurs vérifiés suspects - Nettoyage forcé'
            );
            ...
            throw new Error(
              "Vous avez déjà configuré l'authentification à deux facteurs. Utilisez la page de gestion MFA pour modifier vos paramètres."
            );
          }
        }
        ...
        if (factorsCount.unverified > 0) {
          console.log(
            '🧹 Nettoyage automatique des facteurs non vérifiés (sauf facteurs récents)...'
          );

          // Nettoyage plus conservateur - ne supprime que les anciens facteurs
          const cleanupResult =
            await MFACleanupUtil.cleanupUnverifiedFactors(false);
          ...
          console.log(
            `✨ Nettoyage terminé: ${cleanupResult.cleaned} facteurs supprimés`
          );
      ...
      const baseName =
        userRole === 'nutritionist' ? 'NutriSensia Pro' : 'NutriSensia';
      ...
      if (enrollError) {
        ...
        if (
          ...
          (enrollError.message?.includes('friendly name') &&
            enrollError.message?.includes('exists'))
        ) {
          console.warn(
            '⚠️ Facteur avec le même nom détecté - nettoyage automatique...'
          );

          // Nettoyage automatique des facteurs avec le même nom
          try {
            const cleanupResult =
              await MFACleanupUtil.cleanupUnverifiedFactors(true);
            console.log(
              '🧹 Nettoyage automatique "already exists":',
              cleanupResult
            );
            setCleanupResult(
              `Nettoyage automatique: ${cleanupResult.cleaned} facteurs supprimés`
            );
            ...
            if (retryCount < 2) {
              console.log(
                '🔄 Nouvel essai après nettoyage... (tentative',
                retryCount + 1,
                '/3)'
              );
              ...
              console.error(
                '❌ Trop de tentatives de nettoyage - arrêt pour éviter boucle infinie'
              );
              throw new Error(
        ...
        if (enrollError.message?.includes('Unexpected failure')) {
          console.warn(
            '⚠️ Erreur inattendue détectée - mais PAS de nettoyage automatique pour éviter de supprimer le facteur en cours'
          );
      ...
      if (data && data.totp) {
        console.log('🔐 Données MFA reçues:', {
          ...
          qrCodeType: data.totp.qr_code?.startsWith('data:')
            ? 'SVG Data URI'
            : 'TOTP URI',
        });
        ...
        if (data.totp.qr_code?.startsWith('data:image/svg')) {
          console.warn(
            "⚠️ Supabase a renvoyé un SVG au lieu d'une URI TOTP, génération manuelle..."
          );
  ...
  const verifyAndFinalize = async () => {
    ...
    try {
      ...
      if (factorsError) {
        console.error(
          '❌ Erreur lors de la vérification des facteurs:',
          factorsError
        );
        throw new Error(
          'Impossible de vérifier les facteurs MFA. Veuillez réessayer.'
        );
      }

      // CORRECTION: Chercher d'abord dans totp, puis dans all si pas trouvé
      let factorExists = factorsData.totp?.some(
      ...
      if (!factorExists && factorsData.all) {
        ...
        if (factorExists) {
          console.warn(
            '⚠️ Facteur trouvé dans "all" mais pas dans "totp" - problème de synchronisation Supabase'
          );
        }
      }
      ...
      if (!factorExists) {
        ...
        console.log(
          '⚠️ Facteur non trouvé - Le facteur a probablement été supprimé par un nettoyage automatique'
        );
        console.log(
          "💡 L'utilisateur doit recommencer la configuration 2FA depuis le début"
        );
      ...
      if (challengeError) {
        ...
        if (
          ...
          throw new Error(
            'Facteur MFA non trouvé. Veuillez recommencer la configuration 2FA.'
          );
        }
              ...
              <Button
                onClick={async () => {
                  ...
                  try {
                    ...
                    setCleanupResult(
                      `Nettoyage d'urgence: ${result.cleaned} facteurs supprimés`
                    );
                    ...
                    console.error(
                      "❌ Erreur nettoyage d'urgence:",
                      cleanupError
                    );
                    setError(
                      'Erreur lors du nettoyage. Veuillez contacter le support.'
                    );
                  } finally {
                    setIsLoading(false);
                  }
                ...
                {error.includes('already exists') && (
                  <Button
                    onClick={async () => {
                      ...
                      try {
                        console.log(
                          "🧹 Nettoyage d'urgence des facteurs MFA..."
                        );
                        const result =
                          await MFACleanupUtil.cleanupUnverifiedFactors(true);
                        console.log("🧹 Résultat nettoyage d'urgence:", result);
