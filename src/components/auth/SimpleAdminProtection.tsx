...
export default function SimpleAdminProtection({ children }: SimpleAdminProtectionProps) {
  ...
  useEffect(() => {
    ...
    async function checkAccess() {
      try {
        ...
        if (!isAdmin) {
          setError(`Accès refusé : permissions administrateur requises (rôle: ${userData.role})`);
          console.log('🔧 SimpleAdminProtection: Accès refusé pour le rôle:', userData.role);
        } else {
          console.log('🔧 SimpleAdminProtection: Accès autorisé pour le rôle:', userData.role);
        }
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center p-6 bg-white rounded-lg shadow-md max-w-md">
          ...
          <p className="text-gray-700 mb-4">
            {error || 'Permissions administrateur requises pour accéder à cette page.'}
          </p>
