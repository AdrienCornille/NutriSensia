...
import { getFrequentMeals, type FrequentMeal } from '@/data/mock-meals-history';
...
export function FrequentMealsSection({
  ...
  const frequentMeals = useMemo(() => {
    let meals = getFrequentMeals(10);

    // Filter by meal type if specified
    if (filterType) {
      meals = meals.filter((meal) => meal.type === filterType);
    }

    return meals.slice(0, maxItems);
  }, [filterType, maxItems]);
...
function FrequentMealItem({ meal, onSelect }: FrequentMealItemProps) {
  ...
  const foodNames = meal.foods.map((f) => f.name).slice(0, 3).join(', ');
