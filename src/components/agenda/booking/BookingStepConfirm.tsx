...
import { appointmentModeConfig, formatAppointmentDate } from '@/types/agenda';
...
export function BookingStepConfirm({
  ...
  const typeData = formData.consultationTypeData;
  const modeConfig = formData.mode
    ? appointmentModeConfig[formData.mode]
    : null;
        ...
        <div className='space-y-3'>
          <div className='flex justify-between'>
            ...
            <span className='font-medium text-gray-800'>
              {typeData?.icon && <span className='mr-1'>{typeData.icon}</span>}
              {typeData?.name_fr || '-'}
            </span>
          </div>
          <div className='flex justify-between'>
            ...
            <span className='font-medium text-gray-800'>
              {formData.selectedDate
                ? formatAppointmentDate(formData.selectedDate)
                : '-'}
            </span>
          </div>
          <div className='flex justify-between'>
          ...
          <div className='border-t border-emerald-200 pt-3 mt-3'>
            <div className='flex justify-between'>
              ...
              <span className='font-bold text-[#1B998B]'>
                {typeData?.default_price || '-'} CHF
              </span>
            </div>
          </div>
        </div>
      ...
      <div className='p-4 bg-gray-50 rounded-lg'>
        <label className='flex items-center gap-3 cursor-pointer'>
          ...
          <span className='text-sm text-gray-700'>
            Recevoir un rappel par email 24h avant
          </span>
        </label>
      </div>
    </div>
