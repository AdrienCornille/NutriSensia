    ...
    <div className="bg-gradient-to-r from-emerald-500 to-teal-500 rounded-xl p-6 text-white">
      <div className="flex items-start justify-between">
        <div>
          ...
          <h2 className="text-2xl font-bold mt-1">{formatAppointmentDate(appointment.date)}</h2>
          <p className="text-emerald-100 mt-1">
            {appointment.time} • {appointment.duration} min • {appointment.typeName}
          </p>
        </div>
      ...
      <div className="mt-4 pt-4 border-t border-white/20">
        <p className="text-emerald-100 text-sm">⏰ {countdown} • Rappel envoyé par email</p>
      </div>
    </div>
  );
}
