 ...
 */
type ImportStep = 'upload' | 'preview' | 'options' | 'confirmation' | 'progress' | 'result';
    ...
    <div className="space-y-6">
      ...
      {metadata && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <h4 className="font-medium text-blue-900 mb-2">Informations d'export</h4>
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <span className="ml-2">{new Date(metadata.exported_at).toLocaleDateString('fr-FR')}</span>
            </div>
            <div>
            ...
            <div>
              ...
              <span className="ml-2">{metadata.gdpr_compliant ? 'Oui' : 'Non'}</span>
            </div>
          </div>
        </div>
      )}
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-sm text-gray-500">
                    ({typeof sectionData === 'object' ? Object.keys(sectionData).length : 1} champ{Object.keys(sectionData).length > 1 ? 's' : ''})
                  </span>
                </div>
                <motion.div
        ...
        <div className="space-y-3">
          {[
            ...
            },
            {
              value: 'skip' as const,
              label: 'Ignorer',
              description: 'Garde les données existantes, importe seulement les nouvelles',
              icon: Shield,
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <span className="font-medium text-gray-900">Valider les données</span>
              <p className="text-sm text-gray-600">Vérifier la cohérence avant import</p>
            </div>
            <button
          ...
          <div className="flex items-center justify-between">
            <div>
              <span className="font-medium text-gray-900">Créer une sauvegarde</span>
              <p className="text-sm text-gray-600">Sauvegarder vos données actuelles avant import</p>
            </div>
            <button
              onClick={() => updateOption('createBackup', !options.createBackup)}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${
        ...
        <div className="flex items-start space-x-2">
          ...
          <div>
            ...
            <p className="text-sm text-yellow-700 mt-1">
              L'import de données peut modifier vos informations existantes. 
              Il est recommandé de créer une sauvegarde avant de procéder.
            </p>
          </div>
        </div>
      </div>
...
export const DataImportWizard = ({ 
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      ...
      setValidationErrors([`Erreur lors de la lecture du fichier: ${error.message}`]);
    }
  };
  ...
  return (
    <div className={`bg-white rounded-lg shadow-xl ${compact ? 'max-w-2xl' : 'max-w-4xl'} mx-auto`}>
      {/* En-tête */}
      <div className="px-6 py-4 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Import de données</h2>
          ...
          {currentStep === 'confirmation' && (
            ...
            >
              ...
              <div className="bg-gray-50 rounded-lg p-6 space-y-4">
                ...
                <div>
                  <h4 className="font-medium text-gray-900 mb-2">Stratégie de conflit</h4>
                  <p className="text-gray-600 capitalize">{importOptions.conflictStrategy}</p>
                </div>

                <div>
                  <h4 className="font-medium text-gray-900 mb-2">Options</h4>
                  <div className="space-y-1 text-sm text-gray-600">
                    <div>Validation: {importOptions.validate ? 'Activée' : 'Désactivée'}</div>
                    <div>Sauvegarde: {importOptions.createBackup ? 'Oui' : 'Non'}</div>
                  </div>
                </div>
              </div>
      ...
      {currentStep !== 'progress' && currentStep !== 'result' && (
        <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
          ...
          >
            ...
            {currentStep !== 'confirmation' && <ChevronRight className="w-4 h-4" />}
          </button>
        </div>
      )}
