    ...
    >
      <div className='flex items-center justify-between relative z-10'>
        <div className='flex-1'>
          <p className='text-sm font-semibold text-gray-600 tracking-wide uppercase mb-2 opacity-90'>
            {title}
          </p>
          <p
            className={`text-3xl font-bold tracking-tight ${currentColor.text} mb-1`}
          >
            {formatValue(value)}
          </p>
          {change !== undefined && (
            ...
            >
              {change >= 0 ? '↗' : '↘'} {change >= 0 ? '+' : ''}
              {change}% vs précédente
            </p>
          )}
        </div>
        <div
          className={`p-4 rounded-xl ${currentColor.icon} group-hover:scale-110 transition-transform duration-300 ml-4`}
        >
          {icon}
        </div>
      </div>
          ...
          >
            ...
            <div className='relative bg-gray-100 rounded-full h-3 overflow-hidden shadow-inner'>
              <motion.div
                initial={{ width: 0 }}
                animate={{
                  width: `${Math.max(((item.value || 0) / referenceValue) * 100, 2)}%`,
                }}
                transition={{
                  duration: 1.2,
                  delay: index * 0.15,
            ...
            {chartData.length === 1 ? (
              ...
              >
                ...
                {chartData.map((item, index) => {
                  ...
                  const textX =
                    60 + textRadius * Math.cos((midAngle * Math.PI) / 180);
                  const textY =
                    60 + textRadius * Math.sin((midAngle * Math.PI) / 180);
    ...
    const loadData = async () => {
      try {
        ...
        const response = await fetch(
          `/api/analytics/onboarding/metrics?${params}`
        );
            ...
            funnel && Array.isArray(funnel)
              ? funnel.map((step, index) => {
                  ...
                  const translateStepName = (stepName: string) => {
                    ...
                    return (
                      stepTranslations[stepName] ||
                      stepName.charAt(0).toUpperCase() +
                        stepName.slice(1).replace(/-/g, ' ')
                    );
                  };
            ...
            },
            {
              ...
              value: Math.max(
                ...
                (overview.totalUsers || 0) -
                  (overview.completedUsers || 0) -
                  (overview.abandonedUsers || 0)
              ),
              color: '#f59e0b',
            },
          ]}
            ...
            <div className='h-72 flex items-end justify-between gap-3 bg-gradient-to-t from-gray-50 to-transparent p-4 rounded-xl'>
              {trends.slice(-7).map((trend, index) => {
                const maxUsers = Math.max(
                  ...trends.slice(-7).map(t => t.usersStarted || 0)
                );
                const height =
                  maxUsers > 0
                    ? ((trend.usersStarted || 0) / maxUsers) * 100
                    : 0;
                const completionHeight =
                  maxUsers > 0
                    ? ((trend.usersCompleted || 0) / maxUsers) * 100
                    : 0;
                    ...
                    <div className='w-full h-40 flex flex-col justify-end relative'>
                      ...
                      <motion.div
                        ...
                        style={{
                          ...
                          minHeight:
                            height - completionHeight > 0 ? '20px' : '0px',
                        }}
                      ...
                      >
                        ...
                        {height - completionHeight > 15 && (
                          <span className='relative z-10 text-xs font-bold text-white drop-shadow-sm'>
                            {trend.usersStarted > 0
                              ? Math.round(
                                  ((trend.usersCompleted || 0) /
                                    (trend.usersStarted || 1)) *
                                    100
                                )
                              : 0}
                            %
